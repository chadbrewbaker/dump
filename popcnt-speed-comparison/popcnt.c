...
uint32_t sum_baseline_u4(const uint64_t *buf, int len) {
  // assert(len % 4 == 0);
  int cnt = 0;
  for (int i = 0; i < len; i += 4) {
    cnt += buf[i];
...
uint32_t sum_asm_baseline(const uint64_t *buf, int len) {
  // assert(len % 4 == 0);
  uint64_t cnt[4];
  for (int i = 0; i < 4; ++i) {
    cnt[i] = 0;
...
// This works as intended with clang, but gcc turns the MOVQ intrinsic into an
// xmm->mem
...
uint32_t simd_baseline(const uint64_t *buf, int len) {
  __m128i temp[3];
  ...
  for (int i = 0; i < len; i += 4) {
    ...
    temp[2] = _mm_add_epi64(temp[0], temp[1]);
    sum_pair = _mm_add_epi64(temp[2], sum_pair);
  }
  return _mm_extract_epi64(sum_pair, 0) + _mm_extract_epi64(sum_pair, 1);
}

uint32_t simd_baseline4(const uint64_t *buf, int len) {
  __m128i temp[8];
  const uint64_t zeros[2] = {0x00, 0x00};
  __m128i sum_pair;
  sum_pair[0] = 0;
  ...
  for (int i = 0; i < len; i += 8) {
    ...
    temp[4] = _mm_add_epi64(temp[0], temp[1]);
    temp[5] = _mm_add_epi64(temp[2], temp[3]);
    temp[6] = _mm_add_epi64(temp[4], temp[5]);

    sum_pair = _mm_add_epi64(temp[6], sum_pair);
  }
  return _mm_extract_epi64(sum_pair, 0) + _mm_extract_epi64(sum_pair, 1);
}
...
int run_and_time_fn(int len, int iterations,
                    uint32_t (*fn)(const uint64_t *buf, int)) {
...
uint32_t ssse3_popcount3(uint8_t *buffer, int chunks16) {
  static char MASK_4bit[16] = {0xf, 0xf, 0xf, 0xf, 0xf, 0xf, 0xf, 0xf,
...
#undef MAX
    ...
    for (n = 0; n < k; n += 4) {
#define body(index)                                                            \
  __asm__ volatile("movdqa  (%%eax), %%xmm0\n"                                 \
                   "movdqa    %%xmm0, %%xmm1\n"                                \
                   "psrlw         $4, %%xmm1\n"                                \
                   "pand      %%xmm6, %%xmm0\n"                                \
                   "pand      %%xmm6, %%xmm1\n"                                \
...
#undef body
  ...
  __asm__ volatile("movhlps   %%xmm5, %%xmm0\n"
                   "paddd     %%xmm5, %%xmm0\n"
                   "movd      %%xmm0, %%eax\n"
                   : "=a"(result));
...
int run_mula_popcnt(int len, int iterations) {
  ...
  //  assert(total == iterations * 3); // Check that we don't have an off by 1
  //  error.
...
int main() {
    ...
    printf("sum baseline unroll 4: %.2f\n",
           gb_per_s(run_and_time_fn(len, iterations, &sum_baseline_u4), len));

    printf("sum asm baseline: %.2f\n",
           gb_per_s(run_and_time_fn(len, iterations, &sum_asm_baseline), len));
    printf("builtin unrolled: %.2f\n",
           gb_per_s(run_and_time_fn(len, iterations, &builtin_popcnt_unrolled),
                    len));
    printf(
        "builtin unrolled32: %.2f\n",
        gb_per_s(run_and_time_fn(len, iterations, &builtin_popcnt_unrolled32),
                 len));
    printf("builtin errata: %.2f\n",
           gb_per_s(run_and_time_fn(len, iterations,
    ...
    printf("simd baseline: %.2f\n",
           gb_per_s(run_and_time_fn(len, iterations, &simd_baseline), len));
